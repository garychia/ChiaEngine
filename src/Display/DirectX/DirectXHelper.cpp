...
bool DirectXHelper::HandleWindowResizing(ComPtr<ID3D11Device> &pDevice, ComPtr<ID3D11DeviceContext> &pContext,
                                         ComPtr<ID3D11RenderTargetView> &pRenderTarget,
                                         ComPtr<IDXGISwapChain> &pSwapChain, ComPtr<ID3D11Texture2D> &pBackBuffer,
                                         D3D11_TEXTURE2D_DESC &backBufferDesc)
{
    ...
    if (FAILED(hr))
    {
        PRINTLN_ERR("DirectXHelper: failed to resize the swap chain.");
        return false;
    }
    ...
    if (FAILED(hr))
    {
        PRINTLN_ERR("DirectXHelper: failed to retrieve the back buffer.");
        return false;
    }

    hr = pDevice->CreateRenderTargetView(pBackBuffer.Get(), NULL, pRenderTarget.GetAddressOf());
    if (FAILED(hr))
    {
        PRINTLN_ERR("DirectXHelper: failed to recreate the render target view.");
        return false;
    }

    pBackBuffer->GetDesc(&backBufferDesc);
    pContext->OMSetRenderTargets(1, pRenderTarget.GetAddressOf(), NULL);
